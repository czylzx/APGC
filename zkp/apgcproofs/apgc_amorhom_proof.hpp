...
namespace AmorHom
{
    ...
    void Prove(PP &pp, Instance &instance, Witness &witness,  std::string &transcript_str,Proof &proof)
    {
        ...
        for(auto j = 0; j < n; j++)
        {
            vec_base_new[j] = instance.vec_pk[j] * (vec_p[j]) + instance.Refresh_CL.Invert() * (vec_p[j+n])
                            + instance.Refresh_CR.Invert() * (vec_p[j+ 2*n])
                            + (instance.Refresh_CL + instance.Sum_CL[j].Invert()) * (vec_p[j+ 3*n]);
            ECPoint temp = (instance.Refresh_CR + instance.Sum_CR[j].Invert());
