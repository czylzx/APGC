...
#include "../nizk/nizk_amorhom_for_solvent.hpp"
...
namespace ACF_1oon_range
{
    ...
    {
        fout << proof.P << proof.S << proof.V << proof.vec_C << proof.zs << proof.k << proof.f_zkdl << proof.Ax << proof.rS << proof.S; 
    ...
    {
        fin >> proof.P >> proof.S >> proof.V >> proof.vec_C >> proof.zs >> proof.k >> proof.f_zkdl >> proof.Ax >> proof.rS >> proof.S; 
    ...
    Proof Prove(PP pp, Instance instance, Witness witness, std::string &transcript_str)
    {
        ...
        amorhom_instance.vec_pk = instance.vec_pk;
        amorhom_instance.vec_CL.resize(n);
        amorhom_instance.vec_CR.resize(n);
        for(auto i=0;i<n;i++){
            amorhom_instance.vec_CL[i] = instance.vec_CL[i] + instance.vec_C_L[i];
            amorhom_instance.vec_CR[i] = instance.vec_CR[i] + instance.vec_C_R[i];
        }
