...
namespace AmorHom
{
    ...
    void Prove(PP &pp, Instance &instance, Witness &witness,  std::string &transcript_str,Proof &proof)
    {
        ...
        for(auto i=0;i<pp.n;i++){
            transcript_str += instance.vec_CL[i].ToByteString() + instance.vec_CR[i].ToByteString() + instance.vec_pk[i].ToByteString();
        }
        ...
        size_t n = pp.n;
        std::vector<BigInt> vec_p = GenBigIntPowerVector(3*n, p);
        std::vector<ECPoint> vec_base_new(n*4,init);
        for(auto j=0;j<n;j++){
            vec_base_new[j] = pp.g * (-vec_p[3*j]) + instance.vec_CR[j] * (-vec_p[3*j+1]) + instance.V * (-vec_p[3*j+2]);
            vec_base_new[j+3*n] = pp.g * vec_p[3*j+2];
        }
        
        size_t index = 0;
        for(auto j=0;j<n;j++)
        {
            vec_base_new[n+index] = instance.vec_pk[j] * vec_p[3*j] + instance.vec_CL[j] * vec_p[3*j+1];
            vec_base_new[n+index+1] = pp.h * ((vec_p[3*j+1] + vec_p[3*j+2]) % order); 
            index = index + 2;
        }
