...
bool Verify(PP &pp, Instance &instance, std::string &transcript_str, Proof &proof)
{
    ...
    // ECPoint LEFT = pp.h * proof.taux + pp.g * proof.tx;
    ...
    // vec_A[n] = pp.g, vec_A[n+1] = proof.T1, vec_A[n+2] = proof.T2;
    ...
    V1 = (LEFT == RIGHT); 
    #ifdef DEBUG
        std::cout << std::boolalpha << "Condition 1 (BulletProof) = " << V1 << std::endl; 
    #endif
    ...
    #ifdef DEBUG
        std::cout << std::boolalpha << "Condition 2 (BulletProof) = " << V2 << std::endl; 
    #endif
